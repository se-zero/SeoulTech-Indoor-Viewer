...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            wdbc.data.append([float(value) for value in items[2:]])         # TODO #1) Add 30 attributes (as floating-point numbers)
        wdbc.data = np.array(wdbc.data)
    return wdbc

if __name__ == '__main__':
    # Load a dataset
    wdbc = load_wdbc_data('week 9/data/wdbc.data')     # TODO #1) Implement 'load_wdbc_data()'

    # Train a model
    model = RandomForestClassifier(n_estimators=100, random_state=42)     # TODO #2) Find a better classifier (SVC accuracy: 0.902)
